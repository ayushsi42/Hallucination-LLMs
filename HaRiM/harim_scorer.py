...
class Harimplus_Scorer:
    def __init__(self,
                 ...
                 mixing_factor: float = 7.0,  # same as lambda in the paper
                 device: str = 'cuda',
                 src_maxlen=1024,
                 tgt_maxlen=110):
    ...
    def _prep_input(self, src_tgt_txts, src_or_tgt='src'):
        ...
        return self._tokenizer(src_tgt_txts, padding=True, truncation=True, max_length=L, return_tensors='pt')
    ...
    def compute(self, predictions: List[str],
        ...
        for mini_s, mini_e, mini_t in tqdm(zip(b_srcs, b_emps, b_tgts), total=len(b_tgts), desc=f"computing HaRiM+ {bsz=}, core={self._pretrained_name}"):
            ...
            src_in, emp_in, tgt_in, tgt_mask = src_in.to(self._device), emp_in.to(self._device), tgt_in.to(self._device), tgt_mask.to(self._device)
            ...
            with torch.no_grad():
                ...
                harim_plus_normalized = ll + self._lambda * harim
